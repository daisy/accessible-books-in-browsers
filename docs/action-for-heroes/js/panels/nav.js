...
async function createNavPanelContents(pathToRoot) {
    ...
    initSearchPanel(document.querySelector("#search"));
...
function convertToTabs() {
    let listOfNavs = document.querySelector("#list-of-navs");
    let listItems = []; // all the tabs

    if (!listOfNavs) {
        // create if doesn't exist
        listOfNavs = document.createElement("nav");
        listOfNavs.setAttribute("id", "list-of-navs");
    ...
    document.querySelector("#nav > div").insertBefore(divOfNavs, listOfNavs);
    listOfNavs.remove();
    divOfNavs.setAttribute("id", "list-of-navs");
...
function createNavPanelTab(label, targetId) {
    ...
    targetWrapper.setAttribute("id", `${targetId}-wrapper`);
    targetWrapper.setAttribute("aria-labelledby", `${targetId}-button`);
    ...
    else if (target.classList.contains("bookmarks")) {
    ...
    let button = 
    `<button 
        aria-controls="${targetId}-wrapper" 
        id="${targetId}-button"
        tab-index="${tabIndex}"
...
function addGoToPage() {
    if (document.querySelector("nav.epubtype-pagelist")) {
        let gotoPage = (pageNumber) => {
            document.querySelector("#gotopage-error").textContent = "";
            ...
            if (page) page.click();
            else document.querySelector("#gotopage-error").textContent = "Page not found";
        };
        ...
        `<div>
            <label for="gotopage-text">Go to page:</label>
            <input id="gotopage-text" type="text" placeholder="Page number"></input>
            <input type="button" value="Go"></input>
        </div>
        <p id="gotopage-error" aria-live="polite"></p>
        `;
        document.querySelector("#pagelist-wrapper").insertBefore(gotoPagelist, document.querySelector("nav.epubtype-pagelist"));

        document.querySelector("#gotopage-text").addEventListener("keyup", e => {
            if (e.code == "Enter") gotoPage(e.target.value);
            else {
                if (e.target.value == "") document.querySelector("#gotopage-error").textContent = "";
            }
        });

        document.querySelector("#gotopage-text ~ input[type=button]").addEventListener("click", e => gotoPage(document.querySelector("#gotopage-text").value));
...
function updateLinks(pathToRoot) {
    ...
    Array.from(document.querySelectorAll("#nav > div a, #nav > div img")).map(elm => {
...
function initSearchPanel(searchPanel) {
    ...
    `<div>
        ...
        <input type="search" id="search-text" placeholder="Search"></input>
...
async function importAboutDoc(pathToRoot) {
    ...
    let navContainer = document.querySelector("#nav #about");
